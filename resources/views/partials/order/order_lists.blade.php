            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    @if ($row->payment->payment_method == 'Transfer Bank' && auth()->user()->role_id == 2)
                        <small>Action </small>
                        <a data-bs-placement="top" class="uploadProof" title="Upload Transfer Proof"
                            data-id="{{ $row->id }}">
                    ...
                    @endif
                    @if ($row->payment->payment_method == 'COD' && auth()->user()->role_id == 1)
                        @if ($row->status_id == 2)
                          <a href="/order/approve_order/{{$row->id}}/{{$row->product_id}}" class="btn btn-success">Approve Order</a>
                          <a href="/order/reject_order/{{$row->id}}/{{$row->product_id}}" class="btn btn-danger">Reject Order</a>
                        @endif
                        @if ($row->status_id == 1)
                          <a href="/order/end_order/{{$row->id}}/{{$row->product_id}}" class="btn btn-primary">End Order</a>
                        @endif
                    @endif
                    @if($row->payment->payment_method == 'COD' && auth()->user()->id == $row->user_id && $row->status_id == 2)
                      <a href="/order/cancel_order/{{$row->id}}" class="btn btn-primary">Cancel Order</a>
                    @endif
                ...
                @if ($row->is_done == '1')
                    <div class="col-md-12">order ends at {{ $row->updated_at->format('d M Y') }} by <span
                            class="link-danger" style="cursor: pointer;">@admin</span>
                    </div>
                @else
                    <div class="col-md-12">order created at {{ $row->created_at->format('d M Y') }} by <a
                            href="{{ $dest }}"
